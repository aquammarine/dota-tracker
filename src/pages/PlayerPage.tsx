...
export default function PlayerPage() {
  ...
  const results = useQueries({
    queries: [
      ...
      { queryKey: ['rating', accountId], queryFn: () => getRating(accountId!) },
    ],
  });

  const [playerQuery, wlQuery, matchesQuery, heroQuery,ratingQuery] = results;
        ...
        <Card className="p-8 bg-neutral-900 border-neutral-800 shadow-xl overflow-hidden relative">
            ...
            <div className="text-center md:text-left">
                ...
                <div className="flex flex-wrap gap-4 text-xs font-bold uppercase tracking-widest text-neutral-500">
                    <span className="flex items-center gap-1">ID: <span className="text-neutral-200 font-mono">{accountId}</span></span>
                    <span className="text-neutral-700">|</span>
                    <span className="flex items-center gap-1 text-blue-400">MMR Estimate: {rating || 'N/A'}</span>
                </div>

                <div className="mt-6 flex gap-8">
                    <div>
                        <p className="text-[10px] uppercase font-bold text-neutral-500 mb-1">Win Rate</p>
                        <p className={`text-3xl font-mono ${Number(winRate) >= 50 ? 'text-green-500' : 'text-red-500'}`}>{winRate}%</p>
                    </div>
        ...
        <Card className="border-neutral-800 bg-neutral-900 shadow-2xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
                ...
                <thead>
                    ...
                    <th className="p-4 text-left">K / D / A</th>
                    <th className="p-4 text-left">Duration</th>
                    <th className="p-4 text-right pr-6">Result</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-neutral-800/50">
                    ...
                    return (
                        ...
                        <td className="p-4 font-bold text-neutral-300">#{match.hero_id}</td>
                        ...
                        <td className="p-4 text-right pr-6">
                            <span className={`px-3 py-1 rounded-md text-[10px] font-black uppercase italic ${
                                isWin ? 'bg-green-500/10 text-green-500 border border-green-500/20' 
